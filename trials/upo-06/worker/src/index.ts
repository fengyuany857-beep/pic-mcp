...
async function ensureSchema(env: Env): Promise<void> {
  await env.AUTH_DB.batch([
    ...
    `),
    env.AUTH_DB.prepare(`
      CREATE TABLE IF NOT EXISTS pixiv_credentials (
...
async function createSession(request: Request, env: Env): Promise<Response> {
  ...
  await env.AUTH_DB.prepare(`
    INSERT INTO session_rate (ip, bucket, count)
    VALUES (?1, ?2, 1)
  ...
  if ((rate?.count ?? 0) > SESSION_RATE_LIMIT_PER_MINUTE) {
    return json(
      { status: "ERROR", error: "RATE_LIMITED", message: "Too many mobile auth sessions." },
      429,
    );
  }

  // Opportunistic cleanup only. Correctness never depends on this cleanup.
  await env.AUTH_DB.batch([
...
async function exchangePixivCode(code: string, codeVerifier: string): Promise<Record<string, any>> {
...
async function completeSession(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    return json({ status: "ERROR", error: "INVALID_JSON", message: "JSON body required." }, 400);
  }
  ...
  try {
    token = await exchangePixivCode(code, codeVerifier);
  } catch {
    return json(
      { status: "ERROR", error: "PIXIV_TOKEN_EXCHANGE_FAILED", message: "Pixiv rejected the authorization code exchange." },
      502,
    );
  }
  ...
  const probeStatus = publicBookmarks.ok && privateBookmarks.ok ? "PASS" : "PARTIAL";

  // Deliberately never return access_token or refresh_token.
  return json({
    status: "CONNECTED",
    source: "pixiv",
    user_id: userId,
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (request.method === "GET" && url.pathname === "/health") {
      return json({ ok: true, service: "pic-mcp-auth-trial06", storage: "cloudflare-d1" });
    }
