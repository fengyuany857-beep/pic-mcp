import baseWorker from "./index-v3";
...
async function completeFromIOSLocalToken(request: Request, env: Env): Promise<Response> {
  ...
  if (!session) {
    return json({ status: "ERROR", error: "SESSION_NOT_FOUND", message: "Unknown mobile auth session." }, 404);
  }
  if (session.consumed) {
    return json({ status: "ERROR", error: "SESSION_ALREADY_USED", message: "Mobile auth session was already used." }, 409);
  }
  if (now >= session.expires_at) {
    return json({ status: "ERROR", error: "SESSION_EXPIRED", message: "Mobile auth session expired." }, 410);
  }
  ...
  // access_token is deliberately not persisted and no token is ever returned.
  return json({
    status: "CONNECTED",
    source: "pixiv",
    user_id: userId,
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (
      request.method === "POST"
      && url.pathname === "/auth/pixiv/mobile/complete-client-token"
    ) {
